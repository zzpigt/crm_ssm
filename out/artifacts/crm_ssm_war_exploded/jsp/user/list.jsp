...
<BODY>
	<FORM id="customerForm" name="customerForm"
		  action="${pageContext.request.contextPath }/user/list.action<c:if test="${queryVo.select}">?select=true</c:if>"
		method=post>
						...
						<TABLE borderColor=#cccccc cellSpacing=0 cellPadding=0
							...
							<TBODY>
											...
											<TBODY>
												<TR>
													...
													<TD><INPUT class=textbox id=sChannel2
														style="WIDTH: 80px" maxLength=50 name="user.userName" value="${queryVo.user.userName}"></TD>
												...
												<TR
													...
													<TD>
														<c:if test="${!queryVo.select}">
															<a href="${pageContext.request.contextPath }/user/toEdit.action?userId=${user.userId}">修改</a>
															&nbsp;&nbsp;
															<a href="${pageContext.request.contextPath }/user/delete.action?userId=${userId.userId}">删除</a>
														</c:if>
														<c:if test="${queryVo.select}">
															<input type="button" value="选择" onclick="clickSelect('${user.userId}', '${userId.userId}')" />
														</c:if>
													</TD>
												</TR>
								...
								<TR>
									<TD><SPAN id=pagelink>
											<DIV
												style="float:right;LINE-HEIGHT: 20px; HEIGHT: 20px; TEXT-ALIGN: right">
